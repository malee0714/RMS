...
<tiles:putAttribute name="title">문서 관리</tiles:putAttribute>
	...
	<div class="subContent">
		<div class="subCon1">
			<h2><i class="fi-rr-apps"></i>${msg.C100000180}</h2> <!-- 고객 배포 자료 목록-->
			<div class="btnWrap">
		...
		<form id="DocForm">
			...
			<div class="subCon1 mgT15" id="detail">
				<h2><i class="fi-rr-apps"></i>${msg.C100000179}</h2> <!-- 문서 정보 -->
				<div class="btnWrap">
				...
				<colgroup>
					...
					<tr>
						...
						<td>
							<input type="text" id="ctmmnyNm" name="ctmmnyNm"  class="wd63p"  readonly>
							...
							<input type="hidden" id="ctmmnySeqno" name="ctmmnySeqno" class="wd100p" style="min-width:10em;">
						</td>
					</tr>
					<tr>
						...
						<td style="text-align:left;">
							<input type="text" id="cycle" name="cycle" class="wd61p numChk">
							<select id="cycleCode" name="cycleCode" style="min-width:0em; width:35%"></select>
							<input type="text" id="date" name="date"  class="wd70p" style="min-width:10em; display:none">
						</td>
			...
			<div class="accordion_wrap mgT17">	
				<div class="accordion">${msg.C100001045}</div>  <!--  고객 배포자료 이력 -->
				<div id="acc1" class="acco_top mgT15" style="display: none">
					<h3>${msg.C100000983}</h3> <!-- 품질 문서 이력 -->
					<!-- 에이유아이 그리드가 이곳에 생성됩니다. -->
					<div class="subCon2">
						<div id="cstmrDdataHistGrid" class="mgT15" style="width:100%; height:300px; margin:0 auto;"></div>
					</div>
				</div>
			</div>
	</div>
	...
	<tiles:putAttribute name="script">
		...
		<script>
			...
			var cstmrDdataHistGrid = 'cstmrDdataHistGrid';
			var dropZoneArea;
			...
			$(document).ready(function(){
				...
				// 고객 배포자료 grid rendering
				setDocMListGrid();

				// 문서이력 grid rendering
				setDocHistGrid();

				// 배포정보 hidden grid rendering
				setWdtbInfoListListGrid();

				// 품질 문서 이력 grid rendering
				setCstmrDdataHistGrid();
				...
				// grid resize
				gridResize(['docMList', 'docHistList',cstmrDdataHistGrid]);
			...
			// selectbox bind
			function setCombo() {
				ajaxJsonComboBox('/qa/getDocSeCombo.lims','docSeSeqnoSch',{docSeNm : "고객배포자료"}, false,null,null,null,function(){
					ajaxJsonComboBox('/qa/getDocSeDetailCombo.lims','docSeDetailSeqnoSch',{docSeSeqno : $("#docSeSeqnoSch").val()}, false, "${msg.C100000480}"); /* 선택 */	
				});
				ajaxJsonComboBox('/qa/getDocSeCombo.lims','docSeSeqno',{docSeNm : "고객배포자료"},  false,null,null,null,function(){
				...
				ajaxJsonComboBox('/com/getCmmnCode.lims','cycleCode', {"upperCmmnCode" : "SY14"}, false,"${msg.C100000480}");  /* 주기 구분 */
			}
			...
			function renderingDialog() {
				...
				dialogMtril("mtrilSch", mtrilData, "cstmrDMtril", function(item){
			...
			function setDocMListGrid(){
				...
				function reformEvent() {
					...
					function (rowIndex, columnIndex, value, item, dataField) {
						...
						// 폐기자 ID가 없으면(=폐기하지 않은 문서면) disabled
				...
				function dsUseEvent() {
					...
					function (rowIndex, columnIndex, value, item, dataField) {  // disabledFunction
						if(item.duspsnId != null && item.duspsnId !="" ) {
							return true;
						}else {
						return false;   
			...
			function setDocHistGrid() {
				...
				AUIGrid.bind(docHistList, "ready", function(event) {
					gridColResize([docHistList], "2");
				});
			}

			// 품질 문서 이력 그리드 세팅
			function setCstmrDdataHistGrid() {
				...
				cstmrDdataHistGrid = createAUIGrid(col2, "cstmrDdataHistGrid", cusPros);

				AUIGrid.bind(cstmrDdataHistGrid, "ready", function(event) {
// 					gridColResize([cstmrDdataHistGrid], "2");
				});
				...
				for (i = 0; i < acc.length; i++) {
					acc[i].addEventListener("click", function() {
						...
						if (panel.style.display === "block") {
							...
							AUIGrid.resize(cstmrDdataHistGrid);

							var seqno = $("#docSeqno").val() != '' ? $("#docSeqno").val() : null;
							getCstmrDdataHist(seqno);
						}
			...
			function dataSave(){
				...
				var saveUrl = "<c:url value='/qa/insCstmrDData.lims'/>";
				...
				customAjax({"url":saveUrl,"data":param,"successFunc":function(data) {
		        	if(data > 0) {
		        		...
		        		if($("#crud").val() == "D") {
		        			...
		        			searchDoc();
							AUIGrid.clearGridData(cstmrDdataHistGrid);
			        		docReset();
		        		}else {
			...
			// 배포자료 삭제
			function dataDel() {
				var url = "<c:url value='/qa/delCstmrDData.lims'/>";
			...
			function searchDoc(keyCode) {
				if(keyCode != undefined && keyCode == 13) {
					...
					if(keyCode == undefined) {
						...
						// 제 * 개정번호, 문서 관리 번호, 문서명 활성화
						disabledEvent();

						// 배포자료 목록 조회
						getGridDataForm("<c:url value='/qa/getCstmrDDataList.lims'/>", "searchFrm", docMList);
					}
				}
			}
			
			// 품질 문서 이력 조회
			function getCstmrDdataHist(seqno) {
				var param = {
					"tableNm" : "RS_CTMMNY_DTA_D"
				};

				if(!!seqno) {
					param.seqno = seqno;
					getGridDataParam('/com//getQlityDocHistList.lims', param, cstmrDdataHistGrid);
				}else {
					return;
				}
			}
				...
				.then(function() {
							detailAutoSet({
								...
								"successFunc" : function() {
									...
									// 문서관리번호로 문서이력 조회
									getGridDataParam("<c:url value='/qa/getCstmrDDataHistList.lims'/>", histParams, docHistList);
									$("#crud").val("U");
									...
									disabledEvent();
									getCstmrDdataHist(event.item.docSeqno);
									var crrDte = $("#date").val()
									var crrCycle = Number($("#cycle").val());
									var crrCycleCode = $("#cycleCode").val();
									var colId = "wdtbPrearngeDt";
			...
			// 문서이력 목록의 선택 or 그리드 더블클릭 시 event
